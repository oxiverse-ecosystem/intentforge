...
- Goals are stored **in-memory** (non-persistent across gateway restarts). `GET /goals/leaderboard` returns a bare JSON **array** of up to 50 entries sorted by score (descending); the count is `len(array)`.
